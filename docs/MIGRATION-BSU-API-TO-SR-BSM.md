...
- `RENDER_DEPLOY_HOOK` - Webhook URL for programmatic deployments (optional, available from Render Settings)

**Note about SR.BSM.env**: The original Render export included a reference to `SR.BSM.env`, but this was a grouping/placeholder in the export format, not an actual environment variable to configure. All required environment variables (CORS_ORIGINS, ADMIN_TOKEN, OPENAI_BSM_KEY, etc.) should be individually configured as shown below.
...
Update existing variables in Render Environment:
...
# Optional: Get deployment webhook URL from Render Settings → Deploy Hook
# Copy the URL for later use, no need to set as environment variable
# RENDER_DEPLOY_HOOK is automatically generated by Render
```

**Note**: No new environment variables need to be added. The `SR.BSM.env` reference in the original export was a placeholder, not an actual variable to configure.
...
Restore old `CORS_ORIGINS`:
```bash
CORS_ORIGINS=https://www.lexdo.uk,https://lexdo.uk,https://lexprim.com,https://www.lexprim.com
```

**Note**: No other environment variables need to be removed since no new ones were added during migration.
