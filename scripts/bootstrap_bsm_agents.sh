...
# Write agent files
cat > .github/agents/bsm-autonomous-architect.agent.md <<'AGENT_EOF'
---
...
cat > .github/agents/orchestrator.agent.md <<'AGENT_EOF'
---
...
cat > .github/agents/runner.agent.md <<'AGENT_EOF'
---
name: BSM Runner
description: >
...
cat > .github/agents/security.agent.md <<'AGENT_EOF'
---
...
cat > .github/PULL_REQUEST_TEMPLATE/agents-suggestions.md <<'TMPL_EOF'
# اقتراحات BSM Agents
...
# Workflow
cat > .github/workflows/agents-run.yml <<'WF_EOF'
name: Run BSM Agents
...
chmod +x scripts/run_agents.sh 2>/dev/null || true

# Commit created files
git add .github/agents .github/PULL_REQUEST_TEMPLATE .github/workflows/agents-run.yml scripts/run_agents.sh scripts/bootstrap_bsm_agents.sh
